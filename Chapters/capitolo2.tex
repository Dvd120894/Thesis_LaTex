...
È importante notare che, come mostrato in Figura \ref{rillimpediment} il modello di contatto di Rill non permette di rilevare ostacoli frontali o comunque al di fuori dell'impronta di contatto.
\begin{figure}
	\centering
	\begin{tikzpicture}
	\def\axisl{3};
...
%
\begin{equation}
\textbf{\textit{e}}_n = \frac{1}{A_{tot}}\sum_{i=0}^{N_T}A_i\textbf{\textit{e}}_{n_i}
\label{ponderata}
\end{equation}
%
dove:
...
È importante notare che, come mostrato in Figura \ref{intersezione} il modello di contatto ponderato in base all'area d'intersezione permette di rilevare ostacoli frontali o comunque al di fuori dell'impronta di contatto.
\begin{figure}
	\centering
	\begin{tikzpicture}
	\def\axisl{3};
...
%
\paragraph{Coefficiente di attrito}
Il coefficiente di attrito $\mu$ viene calcolato come media ponderata dei coefficiente di attrito $\mu_i$ sull'area d'intersezione $A_i$ dell'$i$-esimo triangolo:
%
\begin{equation}
\mu = \frac{1}{A_{tot}}\sum_{i=0}^{N_T}\mu_iA_i
\end{equation}
%
dove:
\begin{itemize}
...
%
\begin{equation}
\textbf{\textit{e}}_{n_{D_j}} = \frac{1}{A_{D_{tot}}}\sum_{i=0}^{N_T}A_i\textbf{\textit{e}}_{n_i}
\end{equation}
%
...
%
\begin{equation}
\textbf{\textit{e}}_n = \frac{1}{A_{tot}}\sum_{j=0}^{N_D}A_{D_j}\textbf{\textit{e}}_{n_{D_j}}
\end{equation}
%
dove:
\begin{itemize}
...
Per combinare assieme i versori normali $\textbf{\textit{e}}_{n_{D_j}}$ relativi ai dischi si effettuerà una nuova media ponderata pesata questa volta sull'area totale d'intersezione all'interno del disco. In questo caso l'area del $j$-esimo disco viene calcolata come piano individuato dalla normale $\textbf{\textit{e}}_{n_{D_j}}$ e dal punto $P_{PL_j}$, calcolato come media dei punto di campionamento del disco. La formula sarà dunque:
%
\begin{equation}
\textbf{\textit{e}}_n = \frac{1}{A_{tot}}\sum_{j=0}^{N_D}A_{D_j}\textbf{\textit{e}}_{n_{D_j}}
\end{equation}
...
\paragraph{Coefficiente di attrito}
Il coefficiente di attrito del singolo disco $\mu_D$ viene calcolato come media aritmetica dei coefficienti di attrito ricavati nei suoi punti di campionamento:
%
\begin{equation}
\mu_D = \frac{1}{N_C}\sum_{i=1}^{4}\mu_i
\end{equation}
...
%
\begin{equation}
\mu = \frac{1}{A_{tot}}\sum_{j=0}^{N_D}A_{D_j}\mu_D
\end{equation}
%
dove:
\begin{itemize}
	\item $N_D$ è il numero di dischi totali rappresentanti lo pneumatico;
	\item $\mu$ è il coefficiente di attrito risultante;
	\item $\mu_D$ è il coefficiente di attrito associato al $j$-esimo disco;
	\item $A_{D_j}$ corrisponde all'area d'intersezione all'interno del $j$-disco e sotto la superficie della \textit{mesh}.
\end{itemize}
