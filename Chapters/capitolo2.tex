\chapter{Modellizzazione dello pneumatico}
\label{Pneumatico}
%
...
Nel caso di pneumatico rappresentato da più dischi, al fine di dare più robustezza e aderenza alla realtà, si è deciso di adottare un metodo per calcolare il punto di contatto differente da quello utilizzato nella sezione \ref{Contatto_Rill}. In altre parole, una volta ottenute le penetrazioni con la strada di ogni disco con il metodo presentato nella sezione \ref{Contatto_Rill}, si andrà a combinarle con una nuova media media ponderata sull'area totale d'intersezione all'interno del disco. La formula sarà dunque:
%
\begin{equation}
...
Avendo ora a disposizione la penetrazione pneumatico/strada risultante si calcolerà il punto di contatto sul piano strada $P_{PL}$ giacente sul piano mediano $XZ$ dello pneumatico. Si troverà dapprima la componente della normale al piano strada $\textbf{\textit{e}}_{n_{XZ}}$ sul piano in cui giace il singolo disco indeformabile. $P_{PL}$ sarà dunque trovato a partire dal centro ruota $M$ come:
%
\begin{equation}
